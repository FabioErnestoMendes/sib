__author__ = "Diogo Cachetas"
__credits__ = ["Diogo Cachetas", "João Capela", "João Correia", "Fernando Cruz", "Vítor Pereira"]
__license__ = "Apache License 2.0"
__version__ = "0.0.1"
__maintainer__ = "Diogo Cachetas"
__email__ = "diogo.cachetas@inl.int"

__author__ = "Fábio Mendes" 
__credits__ = ["Diogo Cachetas", "João Capela", "João Correia", "Fernando Cruz", "Vítor Pereira","Fábio Mendes"]
__license__ = "Apache License 2.0"
__version__ = "0.0.1"
__maintainer__ = "Fábio Mendes"
__email__ = "mendesfabioernesto13@gmail.com"



